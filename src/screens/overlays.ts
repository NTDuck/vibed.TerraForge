...
function connectCard(): HTMLElement {
  return h('div', { class: 'panel' },
    h('h2', { style: 'margin-top:0' }, 'Connect wallet (mockup)'),
    h('p', { class: 'sm muted' }, 'Pick a demo persona. No keys, no chain — state lives in localStorage.'),
    h('div', { class: 'flow' },
      ...ROLES.map((r) =>
        h('button', {
          class: 'btn',
          onclick: () => {
            ...
            commit((s) => ({ ...s })); // no-op: name captured before toast for readability
    ...
    h('tbody', {},
      ...s.txs.map((t) =>
        h('tr', { class: t.status === 'reverted' ? 'reverted' : undefined },
          ...
          h('td', { class: 'mono' }, t.kind),
          h('td', {}, t.label),
          h('td', { class: 'sm muted' }, t.detail),
  ...
  return backdrop(h('div', { class: 'panel wide' },
    h('div', { class: 'row spread' },
      ...
      h('div', { class: 'row' },
        ...
        reverted > 0 ? h('span', { class: 'chip chip-err' }, `${reverted} reverted`) : null)),
    s.txs.length === 0
